...
function Library:_setMinimized(minimized: boolean)
	...
	if minimized then
		...
		if self.BrandMark then
			...
			self.BrandMark.Position = UDim2.fromOffset(14, 29)
		end
		if self.Brand then
			self.Brand.AnchorPoint = Vector2.new(0, 0.5)
			self.Brand.Position = UDim2.fromOffset(56, 29)
...
function Library:Button(config: {[string]: any})
	...
	button.AnchorPoint = Vector2.new(1, 0.5)
	button.Position = UDim2.new(1, -14, 0.5, 0)
	button.Size = UDim2.fromOffset(config.Width or 112, config.Height or 28)
	button.Parent = row
	corner(button, 8)
